...
<div class="cdekfw-pvz-block">
	...
	<select name="cdekfw-pvz-code" id="cdekfw-pvz-code"
		...
		<?php foreach ( $pvz_list as $item ) : ?>
			...
			<option value="<?php echo esc_attr( $pvz_value ); ?>" <?php selected( $pvz_value, $selected_pvz ); ?> data-search-term="<?php echo esc_attr( mb_strtolower( $item['name'] . ' ' . $item['address'] . ' ' . $item['nearest_station'] ) ); ?>">
				<?php
				if ( $item['fullAddress'] ) {
